...
// Update the type for history items to include all fields from the new schema
// (id, expression, result, created_at, updated_at, memory_value, tags, notes, device_id, deleted_at, user_id)
...
const Calculator: React.FC = () => {
    ...
    useEffect(() => {
        if (user) {
            supabase
                ...
                .select("id, expression, result, created_at, updated_at, memory_value, tags, notes, device_id, deleted_at, user_id")
                .eq("user_id", user.id)
                .is("deleted_at", null)
            ...
            setHistory(
                (loadFeatureState<CalculatorPersistentState>(FEATURE_KEY)?.history || []).map((h, i) => ({
    ...
    const handleEqualsClick = useCallback(() => {
        ...
        if (typeof result === "string") {
            ...
            if (user) {
                supabase.from("calculator_history").insert({
                    ...
                    result: result,
                    memory_value: memory, // Save memory value if desired
                    tags: null,           // Add tags if you want to support them
                    notes: null,          // Add notes if you want to support them
                    device_id: null,      // Add device_id if you want to support it
                    deleted_at: null,
                }).then(() => {
                    // Refresh history
                    supabase
        ...
        setOperator(null);
        setWaitingForOperand(true);
    }, [displayValue, operand, operator, isError, waitingForOperand, user, memory]);
    ...
    const handleDeleteHistory = async (itemIndexOrId: number | string) => {
        if (user) {
            ...
            await supabase.from("calculator_history").update({ deleted_at: new Date().toISOString() }).eq(
                "id",
