...
import { focusWindowAtom } from "@/atoms/windowAtoms";
...
const SimpleCalendar: React.FC<SimpleCalendarProps> = ({ windowId }) => {
    ...
    events.forEach((ev) => {
        if (!eventDotMap[ev.date]) {
            eventDotMap[ev.date] = ev.color || "#3b82f6";
        }
    });
    ...
    const handleAddEvent = async (ev: Omit<CalendarEvent, "id">) => {
        await addEvent(ev);
        setModalOpen(false);
    };
    ...
    const renderWeekSidebar = (weekDays: Date[]) => {
        ...
        weekDays.forEach((date) => {
            ...
            weekEvents[dateStr] = events.filter((ev) => ev.date === dateStr);
        });
        return (
    ...
    const renderMonthSidebar = () => {
        ...
        monthDays.forEach((date) => {
            const dateStr = format(date, "yyyy-MM-dd");
            monthEvents[dateStr] = events.filter((ev) => ev.date === dateStr);
        });
        return (
            <div className="fixed right-0 top-0 h-full w-80 bg-background border-l border-border shadow-lg z-[9999] flex flex-col p-4 transition-transform">
                <div className="flex items-center justify-between mb-2">
